...
function loadQuestion() {
  ...
  const qObj = list[currentIndex];
  questionText.textContent = qObj.q;
  questionCounter.textContent = `Question ${currentIndex + 1} / ${list.length}`;
  ...
  shuffledOptions.forEach((opt) => {
    ...
    btn.textContent = opt;
    btn.addEventListener("click", () => handleAnswer(btn, opt, qObj.a));
...
function endCategory() {
  ...
  finalScoreSpan.textContent = `${currentScore} / ${questions[currentCategory].length}`;
  finalBestScoreSpan.textContent = getBestScore(currentCategory);
  bestScoreSpan.textContent = getBestScore(currentCategory);
